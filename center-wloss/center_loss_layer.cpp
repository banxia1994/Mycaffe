...
void CenterLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  distance_back_.ReshapeLike(*bottom[0]);
...
void CenterLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  Dtype* distance_back_data = distance_back_.mutable_cpu_data();
  ...
  for (int i = 0; i < M_; i++) {
    ...
    caffe_copy(K_, distance_data + i * K_, distance_back_data + i * K_);
    // lack calculate for j
    for(int j = 0; j < K_; j++){
	    (distance_data[i*K_+j] )= sqrt(wgallery[label_value])*(distance_data[i*K_+j]);//sqrt(w)
      (distance_back_data[i*K_+j] )= wgallery[label_value]*(distance_back_data[i*K_+j]);//sqrt(w)
	 } //Dtype* wNew;
	 //caffe_sqr (1,wgallery+label_value,wNew);
	  //const int i_ = 1;
	    //  caffe_sqr (1,wgallery,wNew);
	  //caffe_scal(1,wNew,distance_data + i*K_);
  }
...
void CenterLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->param_propagate_down_[0]) {
    ...
    const Dtype* distance_data = distance_back_.cpu_data();
      ...
      for (int m = 0; m < M_; m++) {
        ...
        if (label_value == n) {
          ...
          caffe_sub(K_, variation_sum_data + n * K_, distance_data + m * K_, variation_sum_data + n * K_);
			//variation_sum_data[n*K_] = variation_sum_data[n*K_]*sqrt(wgallery[label_value]);
		  //Dtype* wNew;
		  //const int i_ = 1;
	      //caffe_sqr (i_,wgallery+label_value,wNew);
	     //caffe_scal(i_,wNew,variation_sum_data + n * K_);
        }
      }
