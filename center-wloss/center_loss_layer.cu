...
__global__ void Compute_distance_data_gpu(int nthreads, const int K, const Dtype* bottom,
	      const Dtype* label, const Dtype* center,const Dtype* wgallery,Dtype* distance_back, Dtype* distance) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    int m = index / K;
    int k = index % K;
    const int label_value = static_cast<int>(label[m]);
    // distance(i) = x(i) - c_{y(i)}
    distance_back[index] = (bottom[index] - center[label_value * K + k])*wgallery[label_value];
    distance[index] = (bottom[index] - center[label_value * K + k])*sqrt(wgallery[label_value]);
  }
}

template <typename Dtype>
__global__ void Compute_center_diff_gpu(int nthreads, const int M, const int K, 
        const Dtype* label, const Dtype* distance, Dtype* variation_sum, const Dtype* wgallery,
        Dtype* center_diff) {
  CUDA_KERNEL_LOOP(index, nthreads) {
    int count = 0;
    for (int m = 0; m < M; m++) {
      ...
      if (label_value == index) {
        ...
        for (int k = 0; k < K; k++) {
          ...
          variation_sum[index * K + k] *= (wgallery[label_value]);
        }
      }
    }
    for (int k = 0; k < K; k++) {
...
void CenterLossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  Compute_distance_data_gpu<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
      CAFFE_CUDA_NUM_THREADS>>>(nthreads, K_, bottom[0]->gpu_data(), bottom[1]->gpu_data(),
                                this->blobs_[0]->gpu_data(),bottom[2]->gpu_data(),distance_back_.mutable_gpu_data(), distance_.mutable_gpu_data());
  Dtype dot;
...
void CenterLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  Compute_center_diff_gpu<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
      CAFFE_CUDA_NUM_THREADS>>>(nthreads, M_, K_, bottom[1]->gpu_data(), distance_back_.gpu_data(), 
                                variation_sum_.mutable_cpu_data(),bottom[2]->gpu_data(), this->blobs_[0]->mutable_gpu_diff());

  if (propagate_down[0]) {
    caffe_gpu_scale(M_ * K_, top[0]->cpu_diff()[0] / M_, 
                             distance_back_.gpu_data(), bottom[0]->mutable_gpu_diff());
  }
  if (propagate_down[1]) {
    LOG(FATAL) << this->type()
